...
export interface CartItemType {
  ...
  susbcriptionId?: string | undefined;
  name: string;
  variantName: string;
  price: number;
}
...
export default function PricingPage() {
  ...
  //const [activeCourseTab, setActiveCourseTab] = useState("movie");
  const [activeCohortTab, setActiveCohortTab] = useState("movie");
  ...
  const [isMemCardSelected, setIsMemCardSelected] = useState(false);

  const [isFullCohortMSelected, setIsFullCohortMSelected] = useState(false);
  const [isFullCohortMEnrolled, setIsFullCohortMEnrolled] = useState(false);
  ...
  useEffect(() => {
    ...
    async function fetchCohortEnrollments() {
      ...
      try {
        ...
        const full = data.some((enrollment) => {
          const variantIds = enrollment.cohort_variant_ids || [];
          // Assuming "full_program_variant_id" is the ID for the full program variant
          return variantIds.includes("full_program");
        });

        if (full) {
          setIsFullCohortMEnrolled(true);
        }
    ...
    async function fetchUserSubscription() {
      ...
      try {
        ...
        setIsSubscribed(subs.status === "active" || subs.status === "trialing" || subs.status === "past_due" || subs.status === "unpaid");
  ...
  function isVariantAlreadyEnrolled(type: "course" | "cohort", variantId: string) {
    if (type === "course") {
      return enrolledCourseVariantIds.has(variantId);
    } else {
      if (isFullCohortMEnrolled) {
        return true;
      }
  ...
  function getPrice(variant?: CourseVariantType | CohortVariantType) {
    if (!variant) return 0;

    if (launchPhase === "phase_1") return variant.price_phase1;
    if (launchPhase === "phase_2") return variant.price_phase2;
  ...
  async function handleSelectVariant(
    ...
    const isFullProgram = variantId === "full_program";

    /** -----------------------------
     * BLOCK selecting anything else when full program already selected
     * ----------------------------- */
    if (isFullCohortMSelected && type === "cohort" && !isFullProgram) {
      toast.error("You have already selected the Full Program. You cannot select individual cohorts.");
      return;
    }

    /** -----------------------------
     * USER SELECTS FULL PROGRAM
     * ----------------------------- */
    if (!isFullCohortMSelected && type === "cohort" && isFullProgram) {
      // Clear ALL cohorts safely (functional update)
      setCartItems(prev => prev.filter(item => item.type !== "cohort"));
      setIsFullCohortMSelected(true);
    }
    ...
    if (!parentData || !variant) return;

    if (launchPhase !== "phase_1" && launchPhase !== "phase_2") return;
    ...
    if (existingItem) {
      ...
      if (isFullProgram) setIsFullCohortMSelected(false);
    ...
    if (isFullProgram) setIsFullCohortMSelected(true);
  }
            ...
            <div className="bg-[#1b1b27] rounded-xl shadow-sm p-6 mb-8">
              ...
              <div className="grid grid-cols-1 place-items-center">
                ...
                >
                  ...
                  <button
                    onClick={() => {
                        if (cartItems.some((i) => i.type === "subscription")){
                          setIsMemCardSelected(false);
                          handleClearCart();
                        }
                        else {
                          setIsMemCardSelected(true);
                          setCartItems([
                            {
                              type: "subscription",
                              //susbcriptionId:"",
                    ...
                    className={`w-full py-3 px-6 rounded-lg font-medium transition-all duration-200 
                      ${
                        isSubscribed
                          ...
                          : isMemCardSelected ? "bg-red-600 hover:bg-red-500 text-white" : "bg-blue-600 hover:bg-blue-500 text-white"
                      }`}
                  >
                    {isSubscribed ? "Already Subscribed" : isMemCardSelected ? "X Selected" : "Subscribe £399"}
                  </button>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                {activeCohort.variants.map((variant) => (
                  <PricingCard
                    ...
                    disabled={
                      !isSubscribed || !isEnrollmentOpen || isVariantAlreadyEnrolled("cohort", variant.id)
                    }
                    isAlreadyEnrolled={isVariantAlreadyEnrolled("cohort", variant.id)}
                    //getPhaseInfo={getPhaseInfo}
                    launchPhase={launchPhase}
                    isFullCohortMSelected={isFullCohortMSelected}
                    setIsFullCohortMSelected={setIsFullCohortMSelected}
                  />
                ))}
              </div>
            )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                {activeCourse.variants.map((variant) => (
                  <PricingCard
                    ...
                    // Disable selection if enrollment is NOT open OR already enrolled
                    disabled={
                      !isSubscribed || !isEnrollmentOpen || isVariantAlreadyEnrolled("course", variant.id)
                    }
            ...
            <div className="sticky top-8">
              ...
              {currentUser && (
                <PaymentComponent
                  ...
                  user={currentUser}
                  cartItems={cartItems}
                  isGlobalEnrollmentOpen={isEnrollmentOpen}
